...
import { loadCartFromBackend, clearCart } from "./cartActions";
...
export const registerChef = (name, kitchensName, email, password, address) => async (dispatch) => {
  try {
    dispatch({
    ...
    const { data } = await axios.post(
      "/api/chefs/signup",
      { name, kitchenName: kitchensName, email, password, address },
      config
    );
...
export const getUserDetails = (id) => async (dispatch, getState) => {
  try {
    ...
    // Also update userLogin.userInfo with fresh user data
    dispatch({
      type: USER_LOGIN_SUCCESS,
      payload: data,
    });
